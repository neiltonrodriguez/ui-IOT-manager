    ...
    <div class="shadow-sm overflow-hidden my-2">
        ...
        <table class="border-collapse table-fixed w-full text-sm">
            ...
            <tbody class="bg-white dark:bg-slate-800">
                <tr class="hover:bg-gray-100 cursor-pointer duration-200" v-for="a in grupoativos" :key="a.id">
                    ...
                    <td class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                        <button @click="detailsGrupoAtivo(a.id)" class="btn">
                        ...
                        <button @click="deleteGrupoAtivo(a.id)" class="btn">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1"
                                stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
...
import { grupoAtivoStore } from '../../stores/grupoativos.js'
...
const grupoAtivoStorage = grupoAtivoStore();
...
export default {
    ...
    methods: {
        checkAllItems() {
            ...
            let arr = []
            if (element.checked) {
                for (let x = 0; x < this.grupoativos.length; x++) {
                    console.log(x);
                    const e = document.getElementById('check' + this.grupoativos[x].id);
                    e.checked = true
                    arr.push(e.value)
                }
                // this.idsForDelete = arr
            } else {
                for (let i = 0; i < this.grupoativos.length; i++) {
                    const e = document.getElementById('check' + this.grupoativos[i].id);
                    e.checked = false
                }
                arr = []
            }
        ...
        getAll(i = null) {
            ...
            const url = `/ativogrupos/?limit=${this.limit}&offset=${this.limit * this.offset}${search}&ordering=-id`;
            http.get(url)
                .then(res => {
                    this.grupoativos = res.data.results
        ...
        async detailsGrupoAtivo(id) {
            this.$router.push('/details-grupoativo/' + id);
