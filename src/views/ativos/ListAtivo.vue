<template>
    <div>
        <Filtro @meu-evento="filtrar" :filterConta="true" :filterEmpresa="true" :filterGrupoSensor="false"
            :filterDepartamento="false" :filterStatus="true" :filterDepartamentoAtivo="true" />
    </div>
                ...
                <tbody class="bg-white dark:bg-slate-800">
                    <tr class="hover:bg-gray-100 cursor-pointer duration-200" v-for="a in ativos" :key="a.id">
                        ...
                        <td @click="detailsAtivo(a.id)"
                            class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                            <template v-if="a.icone == null"><img
                                    class="rounded-lg shadow-md duration-200 hover:scale-105" width="40"
                                    src="../../assets/img/sem-foto.png"> </template>
                            <template v-else>
                                <img class="rounded-lg shadow-md duration-200 hover:scale-105" :src="a.icone"
                                    width="40">
                            </template>
                        ...
                        <td
                            class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                            <button @click="detailsAtivo(a.id)" class="btn">
                            ...
                            <button @click="deleteAtivo(a.id)" class="btn">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1"
                                    stroke="currentColor" class="w-6 h-6">
                                    <path stroke-linecap="round" stroke-linejoin="round"
...
export default {
    ...
    methods: {
        getAll(i = null) {
            ...
            let departamento = ""
            if (this.filter.departamentoAtivo != "") {
                departamento = `&departamento=${this.filter.departamentoAtivo}`
                this.pages = []
            }
            let empresa = ""
        ...
        checkAllItems(){
            ...
            let arr = []
            if (element.checked){
                for(let x = 0; x < this.ativos.length; x++){
                    console.log(x);
                   const e = document.getElementById('check' + this.ativos[x].id);
                   e.checked = true
                   arr.push(e.value)
                }
                ...
                for(let i = 0; i < this.ativos.length; i++){
                   const e = document.getElementById('check' + this.ativos[i].id);
                   e.checked = false
                }
                arr = []
            }
        ...
        deleteSelecionados() {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    http.delete('/ativos/itens/delete/', this.items)
                        .then(res => {
                            this.getAll();
                            this.$swal.fire({
                                icon: 'success',
        ...
        async detailsAtivo(id) {
            this.$router.push('/details-ativo/' + id);
