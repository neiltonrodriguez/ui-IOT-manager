<template>
    ...
    <div class="container-fluid p-5">
        ...
        <form class="row g-3" v-on:submit.prevent="store(sensorgrupo)">
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" v-model="sensorgrupo.descricao" v-on:keyup="habilitarSalvar()"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400  disabled:bg-red-50 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="">
                </div>
                <div>
...
export default {
    ...
    methods: {
        store(sen) {
            const sg = {
                nome: sen.nome,
                descricao: sen.descricao,
                is_active: sen.is_active
            }
            http.put('/sensorgrupos/' + this.sensorgrupo.id + '/', sg)
                .then(res => {
                    this.$swal.fire({
                        icon: 'success',
                    ...
                    router.push({ name: 'gruposensores' })
                })
                .catch(e => {
        ...
        setSensorGrupo() {
            http.get('/sensorgrupos/' + this.$route.params.id)
                .then(res => {
                    this.sensorgrupo = res.data
                })
                .catch(e => {
                    this.$swal("Oops...", e.response.data.detail, "error");
                    if (e.response.data.detail == "Você não tem permissão para executar essa ação.") {
