<template>
    ...
    <div class="shadow-sm overflow-hidden my-2">
        <div class="py-4">
            <button @click="accessRoute('create-gatewayiot')"
        ...
        <table class="border-collapse table-fixed w-full text-sm">
            ...
            <tbody class="bg-white dark:bg-slate-800">
                <tr class="hover:bg-gray-100 cursor-pointer duration-200" v-for="m in gatewayiot" :key="m.id">
                    ...
                    <td class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                        <button @click="detailsGatewayIot(m.id)" class="btn">
                        ...
                        <button @click="deleteGatewayIot(m.id)" class="btn">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1"
                                stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
...
export default {
    ...
    methods: {
        ...
        checkAllItems() {
            ...
            let arr = []
            if (element.checked) {
                for (let x = 0; x < this.gatewayiot.length; x++) {
                    console.log(x);
                    const e = document.getElementById('check' + this.gatewayiot[x].id);
                    e.checked = true
                    arr.push(e.value)
                }
                // this.idsForDelete = arr
            } else {
                for (let i = 0; i < this.gatewayiot.length; i++) {
                    const e = document.getElementById('check' + this.gatewayiot[i].id);
                    e.checked = false
                }
                arr = []
            }
        ...
        deleteSelecionados() {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    http.post('/gatewayiot/itens/delete/', this.items)
                        .then(res => {
                            this.getAll();
                            this.$swal.fire({
                                icon: 'success',
        ...
        async detailsGatewayIot(id) {
            this.$router.push('/details-gatewayiot/' + id);
