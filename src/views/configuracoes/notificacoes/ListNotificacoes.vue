    ...
    <div class="shadow-sm overflow-hidden my-2">
        ...
        <table class="border-collapse table-fixed w-full text-sm">
            ...
            <tbody class="bg-white dark:bg-slate-800">
                <tr class="hover:bg-gray-100 cursor-pointer duration-200" v-for="n in notificacoes" :key="n.id">
                    <td class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                        <input type="checkbox" :id="'check' + n.id" :value="n.id" v-model="idsForDelete">
                    </td>
...
export default {
    ...
    methods: {
        checkAllItems() {
            ...
            let arr = []
            if (element.checked) {
                for (let x = 0; x < this.notificacoes.length; x++) {
                    console.log(x);
                    const e = document.getElementById('check' + this.notificacoes[x].id);
                    e.checked = true
                    arr.push(e.value)
                }
                ...
                for (let i = 0; i < this.notificacoes.length; i++) {
                    const e = document.getElementById('check' + this.notificacoes[i].id);
                    e.checked = false
                }
                arr = []
            }
        ...
        deleteSelecionados() {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    http.post('/notificacao/itens/delete/', this.items)
                        .then(res => {
                            this.getAll();
                            this.$swal.fire({
                                icon: 'success',
        ...
        getAll(i = null) {
            ...
            const url = `/notificacao/?limit=${this.limit}&offset=${this.limit * this.offset}${search}${is_active}&ordering=-id`;
            http.get(url)
                .then(res => {
