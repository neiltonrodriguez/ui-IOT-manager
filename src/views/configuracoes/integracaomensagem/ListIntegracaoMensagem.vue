 ...
 @click="deleteAllSelected()" class="px-4 mx-3 py-2 font-semibold text-sm bg-red-800 disabled:bg-gray-300 text-white rounded-md shadow-sm">Excluir Selecionados</button>
        ...
        <table class="border-collapse w-full text-sm">
            ...
            <tbody class="bg-white dark:bg-slate-800">
                <tr class="hover:bg-gray-100 cursor-pointer duration-200" v-for="i in mensagemintegracao" :key="i.id">
                    ...
                    <td class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                        <button @click="detailsIntegracaoMensagem(i.id)" class="btn">
                        ...
                        <button @click="deleteIntegracaoMensagem(i.id)" class="btn">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1"
                                stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
    ...
    methods: {
        ...
        deleteAllSelected() {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    http.post('/mensagemintegracao/itens/delete/', this.items)
                        .then(res => {
                            this.getAll();
                            this.$swal.fire({
                                icon: 'success',
        ...
        deleteIntegracaoMensagem(id) {
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    http.delete('/mensagemintegracao/' + id)
                        .then(res => {
        ...
        async detailsIntegracaoMensagem(id) {
            this.$router.push('/details-integracaomensagem/' + id);
