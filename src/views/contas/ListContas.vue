    ...
    <div class="shadow-sm overflow-hidden my-2">
        ...
        <table class="border-collapse table-fixed w-full text-sm">
            ...
            <tbody class="bg-white dark:bg-slate-800">
                <tr class="hover:bg-gray-100 cursor-pointer duration-200" v-for="c in contas " :key="c.id">
                    <td class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                        <input type="checkbox" :id="'check' + c.id" :value="c.id" v-model="idsForDelete">
                    </td>
                    <td @click="viewConta(c.id)"
                        ...
                        <template v-else>
                            <img class="rounded-lg shadow-md duration-200 hover:scale-105" :src="c.logo" width="40">
                        </template>
                    </td>
                    <td @click="viewConta(c.id)"
                        class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
...
deleteAllSelected() {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    http.post('/contas/itens/delete/', this.items)
                        .then(res => {
                            this.getAll();
                            this.$swal.fire({
                                icon: 'success',
