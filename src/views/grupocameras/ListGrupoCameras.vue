<template>
    ...
    <div class="shadow-sm overflow-hidden my-2">
        <div class="py-4">
            <button @click="accessRoute('create-grupocamera')"
                class="px-4 py-2 font-semibold text-sm bg-blue-800 text-white rounded-md shadow-sm">Novo Grupo de
                Camera</button>
        ...
        <table class="border-collapse table-fixed w-full text-sm">
            ...
            <tbody class="bg-white dark:bg-slate-800">
                <tr class="hover:bg-gray-100 cursor-pointer duration-200" v-for="ca in grupocameras" :key="ca.id">
                    ...
                    <td class="border-b border-slate-100 dark:border-slate-700 p-2 text-slate-500 dark:text-slate-400">
                        <button @click="detailsGrupoCamera(ca.id)" class="btn">
                        ...
                        <button @click="deleteGrupoCamera(ca.id)" class="btn">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1"
                                stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
...
export default {
    ...
    methods: {
        ...
        checkAllItems() {
            ...
            let arr = []
            if (element.checked) {
                for (let x = 0; x < this.grupocameras.length; x++) {
                    const e = document.getElementById('check' + this.grupocameras[x].id);
                    e.checked = true
                    arr.push(e.value)
                }
                
            } else {
                for (let i = 0; i < this.grupocameras.length; i++) {
                    const e = document.getElementById('check' + this.grupocameras[i].id);
                    e.checked = false
                }
                arr = []
            }
        ...
        deleteAllSelected() {
            ...
            this.$swal.fire({
                ...
                if (result.isConfirmed) {
                    http.post('/cameragrupos/itens/delete/', this.items)
                        .then(res => {
                            this.getAll();
                            this.$swal.fire({
                                icon: 'success',
        ...
        getAll(i = null) {
            ...
            const url = `/cameragrupos/?limit=${this.limit}&offset=${this.limit * this.offset}${search}&ordering=-id`;
            http.get(url)
                .then(res => {
        ...
        async detailsGrupoCamera(id) {
            this.$router.push('/details-grupocamera/' + id);
